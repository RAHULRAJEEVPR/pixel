...
const loadDashboard=async(req,res)=>{
    try {
const orderData= await orderModel.find({}).sort({date:-1}).populate("product.productId").populate("userId")
      ...
      const UPI=await orderModel.find({paymentType:"upi",status:"Delivered"}).count()
      const COD=await orderModel.find({paymentType:"cod",status:"Delivered"}).count()
      const WALLET=await orderModel.find({paymentType:"wallet",status:"Delivered"}).count()
...
if(req.body.from==""||req.body.to==""){
 ...
 const ss=await orderModel.find({status:"Delivered", date: {
